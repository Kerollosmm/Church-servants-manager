...
void main() {
  ...
  test(
    'updateTeam propagates team name and group to linked students and users',
    () async {
      const originalTeam = TeamModel(
        id: 'team-1',
        name: 'Old Team',
        groupId: 'year1',
      );
  ...
  test(
    'deleteTeam archives the team even when students are still assigned',
    () async {
  ...
  test(
    'deleteTeam clears assigned servant references and archives team',
    () async {
      ...
      expect(teamDoc.data()!['isArchived'], isTrue);
      expect(List<String>.from(servantDoc.data()!['assignedTeamIds']), [
        'team-2',
      ]);
  ...
  test(
    'restoreTeam marks team active again without servant assignment',
    () async {
      const team = TeamModel(
        id: 'team-1',
        name: 'Team',
