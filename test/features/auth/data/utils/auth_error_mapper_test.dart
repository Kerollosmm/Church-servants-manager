  ...
  group('AuthErrorMapper unit tests', () {
    group('FirebaseAuthException mapping', () {
      test('maps user-not-found to UserNotFoundFailure', () {
        ...
        expect(AuthErrorMapper.mapException(e), equals(const UserNotFoundFailure()));
      });

      test('maps wrong-password, invalid-credential, and INVALID_LOGIN_CREDENTIALS to WrongPasswordFailure', () {
        expect(
          AuthErrorMapper.mapException(FirebaseAuthException(code: 'wrong-password')),
          equals(const WrongPasswordFailure()),
        );
        expect(
          AuthErrorMapper.mapException(FirebaseAuthException(code: 'invalid-credential')),
          equals(const WrongPasswordFailure()),
        );
        expect(
          AuthErrorMapper.mapException(FirebaseAuthException(code: 'INVALID_LOGIN_CREDENTIALS')),
          equals(const WrongPasswordFailure()),
        );
      });
      ...
      test('maps email-already-in-use to EmailAlreadyInUseFailure', () {
        ...
        expect(AuthErrorMapper.mapException(e), equals(const EmailAlreadyInUseFailure()));
      });

      test('maps weak-password to WeakPasswordFailure', () {
        final e = FirebaseAuthException(code: 'weak-password');
        expect(AuthErrorMapper.mapException(e), equals(const WeakPasswordFailure()));
      });

      test('maps invalid-email to InvalidEmailFailure', () {
        final e = FirebaseAuthException(code: 'invalid-email');
        expect(AuthErrorMapper.mapException(e), equals(const InvalidEmailFailure()));
      });
      ...
      test('maps user-disabled to GenericAuthFailure with custom message', () {
        ...
        expect((failure as GenericAuthFailure).message, contains('تم إيقاف هذا الحساب'));
      });

      test('maps too-many-requests to GenericAuthFailure with throttling message', () {
        final e = FirebaseAuthException(code: 'too-many-requests');
        final failure = AuthErrorMapper.mapException(e);
        expect(failure, isA<GenericAuthFailure>());
        expect((failure as GenericAuthFailure).message, contains('لقد حاولت عدة مرات'));
      });

      test('maps unhandled FirebaseAuthException code using message or code', () {
        final eWithMessage = FirebaseAuthException(code: 'custom-error', message: 'Custom message');
        final failureWithMessage = AuthErrorMapper.mapException(eWithMessage);
        expect((failureWithMessage as GenericAuthFailure).message, equals('Custom message'));

        final eWithoutMessage = FirebaseAuthException(code: 'unknown-code');
        final failureWithoutMessage = AuthErrorMapper.mapException(eWithoutMessage);
        expect((failureWithoutMessage as GenericAuthFailure).message, equals('unknown-code'));
      });
    });
    ...
    group('Custom AuthException mapping', () {
      ...
      test('maps ArchivedAccountAuthException', () {
        ...
        expect((failure as ArchivedAccountFailure).message, equals('Account archived'));
      });
      ...
      test('maps PasswordResetAuthException', () {
        ...
        expect((failure as PasswordResetFailure).message, equals('Reset error'));
      });
      ...
      test('maps GenericAuthException', () {
        ...
        expect(failure, isA<GenericAuthFailure>());
        expect((failure as GenericAuthFailure).message, equals('Generic error'));
      });
    });

    group('Direct AuthFailure and unknown error mapping', () {
      test('returns AuthFailure directly if passed an AuthFailure instance', () {
