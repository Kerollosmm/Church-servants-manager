...
class SyncService {
  ...
  /// Adds a list of mutation entries to the sync queue in a single batch.
  Future<void> enqueueBatch(List<SyncEntry> entries) async {
    if (entries.isEmpty) return;
    if (_activeUserId == null) {
      developer.log(
        'Warning: Attempted to enqueue SyncEntries when no user is authenticated',
        name: 'SyncService',
      );
      return;
    }
    try {
      ...
      final Map<String, SyncEntry> stampedEntries = {};
      for (final entry in entries) {
        stampedEntries[entry.id] = SyncEntry(
          id: entry.id,
          actionType: entry.actionType,
          payload: entry.payload,
          createdAt: entry.createdAt,
      ...
      await box.putAll(stampedEntries);
      _statusController.add(SyncEngineStatus.idle());
      ...
      developer.log(
        'Failed to enqueue batch of SyncEntries',
        error: e,
        stackTrace: stack,
        name: 'SyncService',
      );
